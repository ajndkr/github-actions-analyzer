    ...
    (file: File) => {
      ...
      Papa.parse(file, {
        ...
        complete: (results) => {
          ...
          if (
            !("workflow_name" in firstRow) ||
            !("quantity" in firstRow) ||
            !("net_amount" in firstRow)
          ) {
            const availableColumns = Object.keys(firstRow).join(", ");
            setError(
              `CSV must contain workflow_name, quantity, and net_amount columns. Found columns: ${availableColumns}`,
            );
            setIsLoading(false);
            return;
          }
          ...
          const workflowStats: Record<
            string,
            { minutes: number; cost: number; runs: number }
          > = {};
    ...
    >
      ...
      <div className="flex flex-col items-center text-center">
        ...
        <p className="mt-2 text-xs text-gray-400">
          CSV should contain workflow_name, quantity (minutes), and net_amount
          (cost) columns
        </p>
      </div>
    </div>
