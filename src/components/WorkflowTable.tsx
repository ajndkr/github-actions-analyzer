            ...
            <tr>
              <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                ...
                >
                  ...
                  <ArrowDownUp size={14} className={`transition-opacity ${sortField === 'workflow_name' ? 'opacity-100' : 'opacity-30'}`} />
                </button>
              </th>
              <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                <button 
                ...
                >
                  <span>Minutes</span>
                  <ArrowDownUp size={14} className={`transition-opacity ${sortField === 'quantity' ? 'opacity-100' : 'opacity-30'}`} />
                </button>
              </th>
              <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                <button 
                ...
                >
                  <span>Cost</span>
                  <ArrowDownUp size={14} className={`transition-opacity ${sortField === 'net_amount' ? 'opacity-100' : 'opacity-30'}`} />
                </button>
              </th>
            </tr>
          </thead>
          <tbody className="bg-white divide-y divide-gray-200">
            {paginatedData.map((workflow, index) => (
              <tr key={index} className="hover:bg-gray-50 transition-colors">
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 max-w-xs truncate" title={workflow.workflow_name}>
